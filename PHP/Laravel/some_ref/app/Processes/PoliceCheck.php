...
const POLICE_CHECK_EXPIRED = 'expired';
const POLICE_CHECK_EXPIRE_IN_7_DAYS = 'expire in 7';
const POLICE_CHECK_EXPIRE_IN_30_DAYS = 'expire in 30';

class PoliceCheck extends Eloquent
{
    protected $connection = 'mongodb';
    protected $collection = 'wfPoliceCheck';
...
//        Debugbar::debug($message);
        ...
        try {
            ...
            $expiryDate = new Carbon($this->Payload->SRAEndDate->NoneUtcDate);
            if ($this->Step == POLICE_CHECK_EXPIRED) {
                $title = $sender->FullnameAndSNumber . ' police check expired on ' . $expiryDate->format('d/m/Y');
            } else if ($this->Step == POLICE_CHECK_EXPIRE_IN_7_DAYS) {
                $title = $sender->FullnameAndSNumber . ' police check will expire in 7 days by ' . $expiryDate->format('d/m/Y');
            } else if ($this->Step == POLICE_CHECK_EXPIRE_IN_30_DAYS) {
            ...
            $notice->topic = 'police-check-'.$this->Step;
            $notice->title = $title;
    ...
    private function hasBeenNotifiedLast24Hours($title, $sender){
        ...
        $data = PoliceCheck::where('title', $title)
